...
// 1. FIXED: Corrected to calculate RAM instead of disk space
router.get("/api/memory", (req, res) => {
  ...
  const formatGB = (bytes) => (bytes / 1024 ** 3).toFixed(2) + "G";

  // Returning the exact keys the frontend script expects
  res.json({
    ...
    <body>
      ...
      <script>
        function updateColors(percent) {
          ...
          else if (percent >= 20) statusEl.innerText = "LOW";
        ...
        async function fetchMemory() {
          try {
            ...
            // These exact keys now match the new /api/memory response
