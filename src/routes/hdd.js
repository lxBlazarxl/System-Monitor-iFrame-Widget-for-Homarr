...
function getDisk(path = "") {
  ...
  const calcTotalBytes = usedBytes + availableBytes;
  const exactPercent = (usedBytes / calcTotalBytes) * 100;
  ...
  return {
    ...
    available: formatGB(availableBytes),
    percent: exactPercent.toFixed(0) + "%",
    percent_num: parseFloat(exactPercent.toFixed(1)),
    mount,
  };
}

router.get("/api/hdd", (req, res) => {
  res.json(getDisk("/DATA/Mount"));
});
